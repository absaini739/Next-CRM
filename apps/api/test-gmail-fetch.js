...
const gmail_service_1 = require("./src/services/email/gmail.service");
...
async function testGmailFetch() {
    try {
        ...
        const inboxResult = await gmail_service_1.gmailService.fetchMessages(account.id, {
            maxResults: 10,
            labelIds: ['INBOX']
        });
        ...
        const sentResult = await gmail_service_1.gmailService.fetchMessages(account.id, {
            maxResults: 10,
            labelIds: ['SENT']
        });
        ...
        const allResult = await gmail_service_1.gmailService.fetchMessages(account.id, {
            maxResults: 10
        });
        console.log(`ALL: Found ${allResult.messages.length} messages`);
        if (allResult.messages.length > 0) {
            allResult.messages.forEach((msg, idx) => {
                const parsed = gmail_service_1.gmailService.parseMessage(msg);
